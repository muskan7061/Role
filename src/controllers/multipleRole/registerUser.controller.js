...
const { registerUser } = require("../../models/multipleRole/registerUser.model");
...
const register = async (req, res) => {
  try {
    ...
    const existUser = await registerUser.findOne({ where: { email } });
    if (existUser) {
      return res.status(409).json({
        status: 409,
    ...
    const userCreate = await registerUser.create({
      username,
      email,
...
const loginUser = async (req, res) => {
  try {
    ...
    const findUser = await registerUser.findOne({ where: { email } });
    if (!findUser) {
      return res.status(409).json({
        status: 409,
